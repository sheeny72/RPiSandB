...
def trplot (tr, j):
    ...
    if plotrs:
        ...
        axtr.set_xlim (pstimes[j][0]-5, pstimes[j][1]+5)
...
def buildStream(strm, op, rp):
    ...
    tr1 = []
    for i in range(0, n):
        """ use this routine if using local inventory files
        ...
        """
        # comment out the next line if using local inventory files
        inv = rs.get_stations(network='AM', station=slist[i], level='RESP')  # get the instrument response
...
for t in st:
    ...
    ax.text(t.stats.distance / 1e3, axt-5, mlabel+' = '+str(np.round(t.stats.mL,1)), rotation=90,
            va="bottom", ha="center", color = 'r', zorder=10)    #print ML estimates
    mLav += t.stats.mL  
    j += 1
...
#plot the map
ax2 = fig.add_subplot(1,2,2, projection=OSM().crs)

# estimate the map width and height in pixels
#mwpixels = 840
mwpixels = 950
mhpixels = 1213

# calculate required resolution for latitude and longitude in degrees per pixel
latp = (maxlat - minlat)/mhpixels
lonp = (maxlon - minlon)/mwpixels

# select the required minimum resolution for the map in degrees per pixel
mapp = max(latp, lonp)

# find the OSM zoom level that best fits the map
for zoom in range(0,20):
    if 360/256/2**zoom < mapp*1.1:
        break
zoom-=1

# calculate the width and height of the map in degrees  
osmp = 360/256/2**zoom
osmlat = osmp*mhpixels
osmlon = osmp*mwpixels
#print(latp, lonp, osmp)

# calculate the map extents to suit the OSM resolution
mt = maxlat+(osmlat-(maxlat-minlat))/2 
mb = minlat-(osmlat-(maxlat-minlat))/2 
mr = maxlon+(osmlon-(maxlon-minlon))/2 
ml = minlon-(osmlon-(maxlon-minlon))/2 
 
#ax2 = plt.axes(projection=imagery.crs)
ax2.set_extent((ml, mr, mb, mt))
#ax2.autoscale_view(None)

# Add the imagery to the map.
ax2.add_image(imagery, zoom, interpolation='spline36')
...
for tr in st:
    ...
    ax2.plot([tr.stats.longitude, lonE], [tr.stats.latitude, latE],
         ...
         transform=ccrs.PlateCarree(), label = tr.stats.station,
         )
    if plotrs:
        # initiate Geodesic
        gd = Geodesic()
...
for pl in places:
    if pl[2]>ml and pl[2]<mr:   #test for longtiude inside the map
        if pl[1]<mt and pl[1]>mb:   #test for latitude inside the map
            ax2.plot(pl[2], pl[1], color='k', marker='o', markersize=pl[3], markeredgecolor='k', transform=ccrs.PlateCarree(), label=pl[0])
            ax2.text(pl[2], pl[1]+20.48*0.5**zoom, pl[0], horizontalalignment='center', transform=ccrs.PlateCarree())
"""
#add Notes
if eq:
...
fig.text(0.51, 0.1,'https://github.com/sheeny72/RPiSandB      Background map imagery: OpenStreetMap', size='x-small', rotation=90)
...
plt.subplots_adjust(wspace=0.1, left = 0.05, right = 0.95, bottom = 0.05)

# print text for social media post
if eq:
    print('M'+str(mag)+' #earthquake detected at '+locE+' on the #RaspberryShake #CitizenScience seismic network. @Raspishake #python #Obspy @Matplotlib #Cartopy #seismology #OCSN')
else:
    print('M'+str(mag)+' #earthquake likely #mineblast at '+locE+' detected on the #RaspberryShake #CitizenScience seismic network. @Raspishake #python #Obspy @Matplotlib #Cartopy #seismology #OCSN')
